...
const firstLetters = (arr) => {
  ...
  const localArr=[];
  arr.forEach(element => {
    localArr.push(element.substring(0, 1));
...
const findHappiness = (arr) => {
  // Solution code here...
  let localArr = arr.filter(val => val.includes(':)'));
  return localArr;
};
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  const regex= /\D/g;
  return arr.map(phoneNumber => phoneNumber.replace(regex, ''));
...
const allHappy = (arr) => {
  // Solution code here...
  let localArr = arr.filter((val) => {
    return val.includes(':)');
  });
  
  let result = localArr.length == arr.length ? true : false;
  return result;
};
